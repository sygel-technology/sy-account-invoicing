...
class AccountPaymentMode(models.Model):
    ...
    report_text = fields.Char(
        string="Text on report",
        help="This text will be displayed on the report with the accounts",
        translate=True,
    )
    invoice_account = fields.Boolean(
        default=True,
        string="Use Invoice Account",
        help="Use the bank account set on the invoice",
    )
    ...
    show_part_bank_account_chars = fields.Integer(string="# of digits")
    show_payment_message_sale = fields.Boolean(
        default=False, string="Show Payment Message in Sales"
    )
    show_payment_message_invoice = fields.Boolean(
        default=False, string="Show Payment Message on Invoices"
    )
